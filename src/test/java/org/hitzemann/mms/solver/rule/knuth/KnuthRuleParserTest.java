...
public final class KnuthRuleParserTest {
	...
	public void testGuessFixedSimpleRuleStrict() {
		...
		when(
				ruleFactoryMock.createGuessFixedSimpleRule(anyInt(),
						any(SpielKombination.class), any(IRule.class)))
				.thenReturn(ruleMock);
		when(ruleFactoryMock.createGuessFirstRule(anyInt(), anyInt()))
		...
		verify(ruleFactoryMock).createGuessFixedSimpleRule(eq(123),
				eq(new SpielKombination(4, 1, 2, 3)), same(nextRuleMock));
	...
	public void testGuessFixedSimpleRuleLenient() {
		...
		verify(ruleFactoryMock).createGuessFixedSimpleRule(eq(3182),
				eq(new SpielKombination(2, 2, 3, 1)), same(nextRuleMock));
		verifyNoMoreInteractions(ruleFactoryMock, ruleMock, nextRuleMock);
	}

	/**
	 * Testet das Parsen einer {@link GuessFixedSimpleRule} mit geklammerter
	 * Ratekombination.
	 */
	@Test
	public void testGuessFixedSimpleRuleBraced() {
		...
		verify(ruleFactoryMock).createGuessFixedSimpleRule(eq(12),
				eq(new SpielKombination(4, 1, 5, 2)), same(nextRuleMock));
		verifyNoMoreInteractions(ruleFactoryMock, ruleMock, nextRuleMock);
	}
	...
	public void testGuessFixedComplexRule() {
		...
		when(
				ruleFactoryMock.createGuessFixedComplexRule(anyInt(),
						any(SpielKombination.class), any(IRule[][].class)))
				.thenReturn(ruleMock);
		when(ruleFactoryMock.createGuessFirstRule(anyInt(), anyInt()))
				.thenReturn(nextRuleMockA, nextRuleMockB, nextRuleMockC,
						nextRuleMockD);
		...
		final ArgumentCaptor<IRule[][]> nextRuleCaptor = ArgumentCaptor
				.forClass(IRule[][].class);
		verify(ruleFactoryMock).createGuessFixedComplexRule(eq(14),
				eq(new SpielKombination(4, 4, 1, 5)), nextRuleCaptor.capture());
		...
		verifyNoMoreInteractions(ruleFactoryMock, ruleMock, nextRuleMockA,
				nextRuleMockB, nextRuleMockC, nextRuleMockD);
	}
