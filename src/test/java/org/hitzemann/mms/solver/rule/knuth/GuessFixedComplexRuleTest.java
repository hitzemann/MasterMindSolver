...
public final class GuessFixedComplexRuleTest {
     ...
     */
    private final IRule[][] nextRules = new IRule[][] { { mock(IRule.class), mock(IRule.class), mock(IRule.class), },
            { mock(IRule.class), mock(IRule.class), }, { mock(IRule.class), }, };

    /**
     * Testet den Konstruktor mit <code>null</code> als zu ratende {@link SpielKombination}.
     */
    @Test
    ...
    /**
     * Testet den Konstruktor mit <code>null</code> als Eintrag im Folge-Regel-Array.
     */
    @Test
    ...
    /**
     * Testet den Konstruktor mit nicht zur Länge der zu ratenden {@link SpielKombination} passendem Folge-Regel-Array.
     */
    @Test
    public void testNonMatchingLength() {
        try {
            new GuessFixedComplexRule(123, new SpielKombination(1, 2, 3), nextRules);
            fail("expected Exception not thrown");
        } catch (IllegalArgumentException e) {
            LOGGER.log(Level.FINEST, "expected exception", e);
        }
    ...
    public void testGetNextRule() {
        ...
        assertSame(nextRules[0][0], underTest.getRuleForResponse(new ErgebnisKombination(0, 2)));
        assertSame(nextRules[0][1], underTest.getRuleForResponse(new ErgebnisKombination(0, 1)));
        assertSame(nextRules[0][2], underTest.getRuleForResponse(new ErgebnisKombination(0, 0)));

        assertSame(nextRules[1][0], underTest.getRuleForResponse(new ErgebnisKombination(1, 1)));
        assertSame(nextRules[1][1], underTest.getRuleForResponse(new ErgebnisKombination(1, 0)));

        assertSame(nextRules[2][0], underTest.getRuleForResponse(new ErgebnisKombination(2, 0)));
    }
}
