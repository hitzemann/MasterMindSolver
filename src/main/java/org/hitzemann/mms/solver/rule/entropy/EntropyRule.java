 ...
 * </p>
 * <p>
 * Die Entropie (mittlerer Informationsgehalt) einer zu ratenden Kombination ist
 * ein Maß für die "Ungewissheit" des zu ihr gehörenden Ergebnisses. Je höher
 * diese Ungewissheit ist, umso höher ist der Informationsgewinn beim Raten der
 * Kombination.
 * </p>
 * <p>
 * Im Folgenden wird angenommen, dass alle Kandidaten für die geheime
 * Kombination gleich wahrscheinlich sind. Ferner sei
 * </p>
 * <ul>
 * <li>f(x,y) := Ergebnis für geratene Kombination x und geheime Kombination y</li>
 * </ul>
 * <p>
 * Der Wert p(S,x,e) sei die Wahrscheinlichkeit des Ergebnisses e für die
 * geratene Kombination x und eine Menge S von Kandidaten für die geheime
 * Kombination. Unter Annahme der oben beschriebenen Gleichverteilung gilt
 * </p>
 * <ul>
 ...
 * </ul>
 * <p>
 * Diese Regel wählt anhand dieses Wertes aus allen (!) Kombinationen eine mit
 * maximaler Entropie für eine gegebene Menge von Kandidaten für die geheime
 * Kombination aus. Existieren mehrere Kombinationen mit gleicher Entropie, wird
 * sofern möglich eine gewählt, die einer der Kandidaten für die geheime
 * Kombination ist.
 * </p>
 * <p>
 * Da die Regel wiederholt angewendet werden soll, liefert
 * {@link #getRuleForResponse(ErgebnisKombination)} einfach die aktuelle
 * Instanz.
 * </p>
 * 
 * @author schusterc
...
public final class EntropyRule implements IRule {
	...
	/**
	 * Ermittelt eine Kombination mit maximaler Entropie, bei gegebener
	 * Kandidatenmenge für die geheime Kombination. Falls es mehrere
	 * Kombinationen mit maximaler Entropie gibt, wird falls möglich eine
	 * gewählt, die Element der Kandidatenmenge.
	 * 
	 * @param candidates
	...
	private SpielKombination determineMaximumEntropyGuess(
		...
		for (SpielKombination guess : allGuesses) {
			...
			if (entropy > maxEntropy + EPSILON
					|| entropy > maxEntropy - EPSILON
					&& candidates.contains(guess)) {
				maxEntropy = entropy;
				result = guess;
			}
		}
	...
	/**
	 * Ermittelt die Entropie für eine zu ratende Kombination, bei gegebener
	 * Kandidatenmenge für die geheime Kombination.
	 * 
	 * @param guess
	...
	/**
	 * Ermittelt die absoluten Häufigkeiten der Ergebnisse für eine zu ratende
	 * Kombination. Die Zuordnung eines konkreten Ergebnisses zu seiner
	 * absoluten Häufigkeit ist für die Berechnung der Entropie irrelevant,
	 * deshalb werden hier nur die absoluten Häufigkeiten als int-Array
	 * zurückgegeben.
	 * 
	 * @param candidates
	 *            Die Kandidatenmenge für die geheime Kombination.
	 ...
	 *            Die zu ratende Kombination.
	 * 
	 * @return Die absoluten Häufigkeiten aller Ergebnisse, ohne Zuordnung zu
	...
	private int[] determineResponseFrequencies(final SpielKombination zuRaten,
		...
		for (SpielKombination geheim : candidates) {
			final ErgebnisKombination ergebnis = calculator.berechneErgebnis(
					geheim, zuRaten);
			final int key = ergebnis.getSchwarz() * (pins + 1)
					+ ergebnis.getWeiss();
			result[key]++;
		}
