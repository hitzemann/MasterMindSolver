...
/**
 * <p>
 * Eine Regel die immer den ersten Kandidaten rät, und als Folge-Regel immer eine {@link GuessFirstRule} liefert.
 * </p>
 * 
 * <p>
 * Ein Solver, der diese Regel ausführt, wird also wiederholt den ersten möglichen Kandidaten raten und anschließend
 * unmögliche Kandidaten elimieren, bis die geheime Kombination erraten wurde.
 * </p>
 * 
 * <p>
 * Diese Regel wird in Knuth's Algorithmus genau dann verwendet, wenn maximal 2 Kandidaten existieren.
 * </p>
 * 
 * @author chschu
 */
public final class GuessFirstRule implements IRule {
     ...
     *            Die Factory zur Erzeugung der Folge-Regeln.
     */
    public GuessFirstRule(final int theMinimumCandidateCount, final int theMaximumCandidateCount,
            final IKnuthRuleFactory theRuleFactory) {
        if (theMinimumCandidateCount > theMaximumCandidateCount) {
            throw new IllegalArgumentException("minimum candidate count is " + theMinimumCandidateCount
                    + ", which is larger than maximum candidate count " + theMaximumCandidateCount);
        }
        if (theMinimumCandidateCount < 0) {
            throw new IllegalArgumentException("minimum candidate count must be at least 0, got "
    ...
    public SpielKombination getGuess(final List<SpielKombination> candidates) {
        ...
        if (candidateCount < minimumCandidateCount) {
            throw new IllegalArgumentException("expected at least " + minimumCandidateCount + " candidates, got "
                    + candidateCount);
        }
        if (candidateCount > maximumCandidateCount) {
            throw new IllegalArgumentException("expected at most " + maximumCandidateCount + " candidates, got "
                    + candidateCount);
        }

        // wir brauchen mindestens einen Kandidaten
        if (candidateCount < 1) {
            throw new IllegalArgumentException("cannot determine a guess if there are no candidates");
        }
