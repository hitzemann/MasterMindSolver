...
/**
 * <p>
 * Eine Regel, die eine feste {@link SpielKombination} rät und abhängig vom damit erzielten Ergebnis verschiedene
 * Folge-Regeln liefert.
 * </p>
 * 
 * <p>
 * Diese Regel wird in Knuth's Algorithmus genau dann verwendet, wenn mehr als 2 Kandidaten existieren und die
 * Kandidatenmenge mit einem Rateversuch nicht in jedem Fall auf maximal 2 Kandidaten reduziert werden kann.
 * </p>
 * 
 * @author chschu
 */
public final class GuessFixedComplexRule implements IRule {
    ...
    /**
     * Das dreieckige zweidimensionale Array der Folge-Regeln. Bei einer Kombinationsgröße von 4 hat das Array die
     * Struktur [[04,03,02,01,00],[13,12,11,10],[22,21,20],[31,30],[40]].
     */
     ...
     *            Die zu ratende {@link SpielKombination}.
     * @param theNextRules
     *            Das dreieckige zweidimensionale Array der Folge-Regeln. Bei einer Kombinationsgröße von 4 hat das
     *            Array die Struktur [[04,03,02,01,00],[13,12,11,10],[22,21,20],[31,30],[40]].
     */
    public GuessFixedComplexRule(final int theExpectedCandidateCount, final SpielKombination theGuess,
            final IRule[][] theNextRules) {
        if (theGuess == null) {
        ...
        if (nextRules.length != size + 1) {
            throw new IllegalArgumentException("expected array of length " + (size + 1) + ", got " + nextRules.length);
        }

        for (int i = 0; i < size; i++) {
            if (nextRules[i] == null) {
                throw new IllegalArgumentException("next rules at index " + i + " must not be null");
            }
            if (nextRules[i].length != size + 1 - i) {
                throw new IllegalArgumentException("expected sub-array of length " + (size + 1 - i) + " at index " + i
                        + ", got " + nextRules[i].length);
            }
        }
    }
