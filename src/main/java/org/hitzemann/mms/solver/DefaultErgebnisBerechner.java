...
public final class DefaultErgebnisBerechner implements IErgebnisBerechnung {
     ...
     * (non-Javadoc)
     * 
     * @see org.hitzemann.mms.solver.IErgebnisBerechnung#berechneErgebnis(org.hitzemann .mms.model.SpielKombination,
     * org.hitzemann.mms.model.SpielKombination)
     */
    @Override
    public ErgebnisKombination berechneErgebnis(final SpielKombination geheim, final SpielKombination geraten) {
        ...
        if (geheim.equals(geraten)) {
            ...
            for (int n = 0; n < geheim.getSpielSteineCount(); n++) {
                if (geheimMap.get(n) && geratenMap.get(n) && geheim.getSpielStein(n).equals(geraten.getSpielStein(n))) {
            ...
            for (int n = 0; n < geheim.getSpielSteineCount(); n++) {
                for (int m = 0; m < geheim.getSpielSteineCount(); m++) {
                    if (m != n) {
                        if (geheimMap.get(n) && geratenMap.get(m)
                                && geheim.getSpielStein(n).equals(geraten.getSpielStein(m))) {
                            geheimMap.put(n, false);
